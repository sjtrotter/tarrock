...
2. **The trouble.** A waiting family's great-aunt is missing from the ledger. Everyone
   at the landing remembers her — but her name is nowhere in Perrin's book, and it has
   ...
   offers.
3. **The search.** Perrin is on his third ledger in three centuries — the earlier two,
...
5. **Restoration.** The name is written into the current ledger properly, both the given
   name and the hated nickname side by side, so she is remembered whole. Perrin steadies.
...
missing name matters *more*, not less: Perrin is now copying the whole book fair onto the
...
|---|---|---|
| Quest resolved (either state) | none — NPC-level only | Perrin's greeting barks warm; the recovered name appears in the ledger prop; no `WS_*` flag is set and no other quest reads this outcome. |
