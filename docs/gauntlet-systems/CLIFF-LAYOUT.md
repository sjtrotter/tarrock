...
## v2 — the leap off an outcrop (director's idea B, with A's backdrop)

![v2](cliff-layout-v2.png)
...
Changes from v1: the plateau is wider and sparser (the director's word); cardinal notches
cut into every terrace edge; the stones gate pinches (rocks crowd the path above and below,
the ambush ground is two tiles deep between them); **the leap is off an OUTCROP** — from the
high terrace's west lobe a narrow spit juts SOUTH over the cloud, the Fool walks out along
it and the tip points down-screen, so the leap reads as a *drop* the way every ALttP ledge
does (the director's option B: "lead to the left, up a little from the start, jump down
off an overhang"). The tip sits left of the spawn and six tiles up from it. Idea A's
backdrop folds in without changing the geometry: the cloud sea around the whole island
carries the Spread drawn faint and far below it, so standing at the tip you look down
through cloud at the world you are about to fall into; the side-view sequence takes over
from the tip.

Why not jump "up" off a north edge: in the top-down grammar up-screen is away/north, not
down a cliff; a north-edge leap reads as walking off the top of the map rather than a
fall, and the Fool's back is to the camera for the most important image in the game. The
outcrop keeps the fall facing us and gives the Cliff an establishing silhouette (a spit of
turf over nothing) that the Spread-below backdrop makes readable.
